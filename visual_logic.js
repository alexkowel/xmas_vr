 ...
 * Thu Dec 19 2019 16:06:46 GMT+0300 (Moscow Standard Time)
...
function play_animation() {
  ...
  if (i <= 1) {
    ...
    operateAnimation('PLAY', 'santa_claus_rig', 0, 200, 'AUTO', 1,
            function() {
      play_animation();
    }, true, false);
    ...
    operateAnimation('PLAY', 'santa_claus_rig', 200, 370, 'AUTO', 1,
            function() {
      play_animation();
    }, true, false);
    ...
    operateAnimation('PLAY', 'santa_claus_rig', 370, 528, 'AUTO', 1,
            function() {
      play_animation();
    }, true, false);
    ...
    operateAnimation('PLAY', 'santa_claus_rig', 528, 727, 'AUTO', 1,
            function() {
      play_animation();
    }, true, false);
...
_pGlob.traverseNonController = function(obj, controller, callback) {
    ...
    for (var i = 0, l = children.length; i < l; i++) {
        ...
        _pGlob.traverseNonController(children[i], controller, callback);
...
_pGlob.xrGetIntersections = function(controller) {
    ...
    _pGlob.traverseNonController(appInstance.scene, controller, function(obj) {
        objList.push(obj);
    });
...
// enterVRMode puzzle
function enterVRMode(refSpace, hoverToClickTimeout, enterCb, exitCb, unAvailableCb) {

    var VR_RETICLE_DEFAULT_DEPTH = 5;

    var _reticleDepth = VR_RETICLE_DEFAULT_DEPTH;
    var _hoverDelayTimer = 0;
    ...
    function onControllerHover() {
        ...
        for (var i = 0; i < controllers.length; i++) {
            ...
            if (intersections.length > 0) {
                ...
                _reticleDepth = 0.95 * intersection.distance;
            } else {
                var obj = null;
            }

            controller.children.forEach(function(child) {
                if (child.name == 'XR_CONTROLLER_RETICLE')
                    child.position.z = -_reticleDepth;
            });

            // TODO: need to be an array in N of controllers > 1
            var prevHovered = _pGlob.hoveredObject;
            var currHovered = obj ? getPickedObjectName(obj) : "";

            if (prevHovered == currHovered) {
                if (hoverToClickTimeout != Infinity && currHovered && !_hoverDelayTimer) {

                    _hoverDelayTimer = window.setTimeout(function() {
                        _pGlob.pickedObject = currHovered;

                        _pGlob.objClickCallbacks.forEach(function(el) {
                            var isPicked = obj && objectsIncludeObj(el.objNames, currHovered);
                            el.callbacks[isPicked ? 0 : 1]();
                        });

                        // not clearing timer var hear, waiting for hovered object change

                    }, 1000 * hoverToClickTimeout);

                }
                return;
            } else if (_hoverDelayTimer) {
                window.clearTimeout(_hoverDelayTimer);
                _hoverDelayTimer = 0;
            }
    ...
    appInstance.initWebXR('immersive-vr', referenceSpace, function() {
        ...
        _pGlob.xrSessionCallbacks.forEach(function(pair) {
            appInstance.xrSession.addEventListener(pair[0], pair[1], false);
        });
        ...
        _pGlob.xrSessionCallbacks.forEach(function(pair) {
            appInstance.xrSession.removeEventListener(pair[0], pair[1], false);
        });
...
checkVRMode(function() {}, function() {});

play_animation();

playSound(loadSound('./jingle_bells.mp3'), true);
volume(loadSound('./jingle_bells.mp3'), 0.2);

registerOnClick('Plane.003', function() {
  enterVRMode('ORIGIN', Infinity, function() {
    changeVis('Plane.003', false);
  }, function() {
    changeVis('Plane.003', true);
  }, function() {});
  // Mobile Hack
  playSound(loadSound('./jingle_bells.mp3'), true);
}, function() {});
